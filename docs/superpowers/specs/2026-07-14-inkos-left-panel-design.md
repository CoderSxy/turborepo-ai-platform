...
- [ ] 默认左栏无搜索框、全选、选择框、书籍摘要、会话数、上下移动/改名/归档/删除文字按钮
- [ ] 每本书为「箭头 + 文件夹图标 + 书名」；展开后才显示会话与新建会话
- [ ] 可同时展开多本书；切书/切会话后对应书保持展开
- [ ] 会话仅标题 + 相对时间；长标题不破布局
- [ ] 改名/删除/归档/还原/排序/批量仍可从菜单或管理模式到达
- [ ] 删除有确认；归档书默认不混入进行中列表
- [ ] 键盘与触屏可访问折叠与菜单；Esc 不误清空 Composer 输入
- [ ] check-types、lint、相关测试通过
