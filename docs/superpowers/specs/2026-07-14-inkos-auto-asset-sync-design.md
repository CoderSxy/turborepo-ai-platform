...
- [x] 新章节生成后摘要、角色、世界观、伏笔相关资产自动写入，`pendingAssetDeltas` 不被新路径增长
- [x] 同一章节重复恢复/合并不重复追加状态与变更事件
- [x] 历史 pending 按顺序迁移；冲突只产生同步诊断
- [x] 右栏无「待确认」字样与确认/忽略日常入口
- [x] `check-types`、`lint`、相关单测通过
